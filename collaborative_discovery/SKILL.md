...
Do not hardcode model choices in the workflow. Use role names such as `synthesis_anchor`, `adversarial_anchor`, `broad_context`, `feasibility`, and `user_advocate`. The default routing keeps the native OpenAI synthesis anchor and Anthropic adversarial anchor mandatory, with Gemini and Kimi assigned to specialist roles, but the mapping is editable in `assets/routing.toml`.
...
Do not write production code in this skill. Do not create implementation tasks in this skill. If implementation details appear, keep them as feasibility notes only.
...
```bash
python3 .agents/skills/collaborative_discovery/scripts/panel_round.py \
  --phase intake \
...
```bash
python3 .agents/skills/collaborative_discovery/scripts/record_native_response.py \
  --phase intake \
...
Before finalizing, run `python3 .agents/skills/collaborative_discovery/scripts/validate_artifacts.py --artifact-dir .codex_workflow/discovery`. If it fails, either complete the missing panel/artifact work or report the failure honestly.
