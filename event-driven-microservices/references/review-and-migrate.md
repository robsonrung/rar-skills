...
Read this on `existing-edm` and `migrate` jobs. For contract findings, also read `references/event-contracts.md` before prescribing a fix.
...
| CDC dumping internal tables as public streams | consumers couple to private models | **event-first** outbox or an eventification job in a *private* namespace, public denormalized entities |
...
Liberation publishes cross-domain data that other systems already need. It is how you *start* a data communication layer. It is not the architecture.
...
- Timestamp output with the *source* `updated_at`, not publish time.
...
Stop conditions for the slice: producer owns the contract; at least one consumer uses only the stream; old point-to-point path for *that* fact is scheduled for removal.
