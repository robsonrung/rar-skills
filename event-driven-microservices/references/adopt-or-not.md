...
**`hybrid`** (default yes) when some facts belong on streams *and* request-response remains for:
...
Small orgs: prefer one modular monolith that *publishes* the important entities. That still installs a data communication layer without paying the full tax.
