...
React compares objects, arrays, and functions **by reference** in `React.memo` props and in hook deps. The inline function passed to `useMemo` / `useCallback` is recreated every render — that is expected. `useCallback(fn)` ≈ `useMemo(() => fn)`. `useMemo` memoizes a *result*; `useCallback` memoizes the *function*.
