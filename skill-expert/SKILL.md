...
- Load-bearing concepts are named with leitwörter the agent will repeat while acting; generic filler verbs (the adverbs-of-effort the repo's leitwörter guard flags) are replaced by named anchors, and shared concepts use the canonical word, not a synonym.
