...
description: Build a whole feature by breaking a plan into tracer-bullet vertical-slice tasks (via the to-tasks skill — acceptance contracts, gate flags, dependencies), then executing the task DAG with implement-and-review per task (independent tasks in parallel), integrating in dependency order, and finishing with a feature-wide full-review, tests/build green. Use to ship/build a multi-task feature end-to-end. For a single scoped task call implement-and-review directly. Distinct from ship — ship is the idea→PR conductor, including the interactive spec/plan phases and PR delivery, while implement-feature is the plan→built-feature DAG executor with no spec phase and no PR.
---
