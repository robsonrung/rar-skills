...
description: Multi-model controlled delivery and implementation workflow. Use when the task plan is ready and the user wants implementation, bug fixing, refactoring, tests, code review, or controlled delivery in the repository.
---
...
4. Red phase. Add or update the failing test first. Run the narrowest command that proves the test fails for the expected reason.
...
```bash
python3 .agents/skills/collaborative_delivery/scripts/panel_round.py \
  --phase task_intake \
...
```bash
python3 .agents/skills/collaborative_delivery/scripts/record_native_response.py \
  --phase task_intake \
...
Before finalizing, run `python3 .agents/skills/collaborative_delivery/scripts/validate_artifacts.py --artifact-dir .codex_workflow/delivery`. If it fails, either complete the missing panel/artifact work or report the failure honestly.
