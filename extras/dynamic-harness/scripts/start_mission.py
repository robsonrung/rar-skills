#!/usr/bin/env python3
"""Create a unique mission workspace and ledger for dynamic-harness manager mode."""
...
def slugify(value: str) -> str:
    ...
    return slug[:72] or "harness-mission"
...
def ledger_template(title: str, mission_id: str, run_dir: Path) -> str:
    ...
    return f"""# Mission Control Ledger
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a unique mission directory with a uniquely named ledger."
    )
    parser.add_argument(
        "--title",
        required=True,
    ...
    parser.add_argument(
        "--root",
        default=".ai-workflow/harness-missions",
