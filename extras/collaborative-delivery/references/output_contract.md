...
See the "Required outputs" list in SKILL.md and `[skill].required_outputs` in `assets/routing.toml`, which is what `shared/scripts/validate_artifacts.py` enforces. Primary output: `execution_log.md`. Artifact directory: `.ai-workflow/panel/delivery`.
