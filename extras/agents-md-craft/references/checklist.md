...
This checklist doubles as the **self-check** the skill runs on its own output and the **audit** rubric for an existing file. Walk every item before the review gate.

## Audit / self-check checklist

1. **Review gate honored** — nothing was written without explicit user approval (incl. `agent_docs/`).
2. **WHAT/WHY/HOW present** — stack+structure, purpose+component roles, and exact build/test/typecheck commands are all covered.
