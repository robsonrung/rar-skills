...
State it as: *"sidecar — the app stays ignorant; the helper is the **coscheduled pair**."*
...
State it as: *"ambassador — the app's **container API** is localhost; sharding / brokering / splitting lives next to it."*
...
State it as: *"adapter — the platform sees one interface; the app keeps its native one."*
...
A change is breaking if an old invocation stops working *or* silently changes load — Burns' `UPDATE_FREQUENCY` going from seconds to milliseconds is a break even though it still parses.
