...
One input item → one reliable output. The reusable piece is the *queue infrastructure*; the user supplies two **container API**s.
...
State it as: *"work queue — source lists, worker consumes a file, Jobs own reliability."*
...
State it as: *"event-driven batch — the graph is copier → filter → …; every hop is a named primitive."*

## Coordinated batch

Use when parallelism has to *rendezvous*, not just hand off.
...
A join is *not* a reduce. It waits; it does not fold values.
...
State it as: *"join — wait for all; reduce — fold now, repeat until one."*
