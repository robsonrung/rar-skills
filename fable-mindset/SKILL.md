---
name: fable-mindset
description: "Think like Claude Fable across the five moments of a working turn — intake (reading the request: act or assess, the mandate), diagnosis (investigating failures: evidence over recognition), decision (choosing an approach: recommendation, not survey), implementation (landing native diffs), and reporting (the final message). One skill, five moments; apply only the moment(s) that match the current situation. Use when the user says 'think like Fable', 'intake this like Fable', or 'apply the Fable mindset'; at the start of any engineering task on Opus or another model; when investigating a bug, test failure, or unexpected behavior; when choosing between technical approaches or an agent produces option lists instead of decisions; when writing code that should read as the codebase's own authors' work (or a diff 'smells like AI'); or when reporting results at the end of a substantive turn. Governs posture, not procedure — not for sharpening half-baked ideas (brainstorm), step-by-step debugging procedure (diagnose), writing the implementation plan (coding-design-plan), evaluating design content (architecture-lens, design-gate, macro-architecture), refactoring existing mess (clean-code), the test-first loop (tdd), or standalone handoff documents (summarize, session-handoff)."
