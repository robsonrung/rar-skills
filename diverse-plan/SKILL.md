---
name: diverse-plan
description: "Plan a code change or feature by exploring several genuinely different approaches IN PARALLEL ACROSS MULTIPLE MODELS, pressure-testing each from multiple angles, then merging the best of all into ONE enriched plan. Triggers: 'diverse plan this', 'branch out this plan', 'explore approaches for', 'what are the different ways to build/fix X', 'enrich the plan'. Use when a code change is non-trivial and worth seeing from multiple angles before committing to one design. Distinct from coding-design-plan (one approach, one model) and models-consensus (deliberation, not planning — this skill is self-contained and never calls it). Produces a plan, not code."
