...
description: Review the data-systems risk of a coding task — turn design ideas into concrete implementation checks, review findings, and verification steps. Use whenever a change touches stored state and async data paths (databases, queues, caches, search indexes, event streams, background jobs), migrations, external APIs, concurrency, retries, consistency, scalability, reliability, observability, or production data risk. Distinct from macro-architecture, which decides data ownership and service decomposition — this lens implements and verifies those choices at the code level, it does not make them.
---
