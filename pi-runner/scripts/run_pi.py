...
store), so seat shims built on this wrapper carry no shared mutable provider
state: there is nothing to lane-isolate and no fallback chain to disable.
Seat-labelled callers (kimi-runner, glm-runner) rely on the runner identity
...
# Seat labels that delegate to this wrapper map to their real vendor here,
