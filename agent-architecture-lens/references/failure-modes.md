...
| The same email/commit/ticket was created twice | A crash between the side effect and the checkpoint replayed the step | Stable key written to state *before* the effect, checked at the top on re-execution |
