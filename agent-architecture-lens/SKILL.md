...
Zero signals fire → recommend keeping the loop and stop. Say it plainly: *"No signal fires; this stays a loop — **a graph is not an upgrade**."* Adding structure here buys latency and testing surface and returns nothing.
...
2. Is the counter incremented *before* the attempt? A crash mid-attempt must still count as an attempt.
...
2. Does each carry a stable key written to state *before* the effect, and checked at the top on re-execution?
