...
7. **Ground in the repo's language and code.** Give every seat the project's domain glossary (`CONTEXT.md` / `CONTEXT-MAP.md`) and relevant ADRs; seats interpret in that canonical vocabulary, **flag terms that conflict with the glossary**, and **cross-check their reading against the actual code** (read-only), surfacing contradictions rather than guessing. (Adapted from `grill-with-docs`.)
...
3. **Collect domain docs.** Look for `CONTEXT.md` (or `CONTEXT-MAP.md` → per-context `CONTEXT.md`) and `docs/adr/`. Pull the glossary and the ADRs relevant to the prompt; these go into the brief so seats interpret in the project's canonical vocabulary. If none exist, note it (interpretations are then language-ungrounded — lower confidence).
4. **Quorum.** Proceed only with **≥3 seats**. With 2 or fewer, stop and tell the user which prerequisites are missing rather than running a thin roundtable.
5. **Artifact mode.** Use `persisted` when `.ai-workflow/` is writable (it is in this repo): write under `.ai-workflow/roundtable/<session_id>/`. Otherwise use `inline` and keep everything in memory, returning paths as `null`.
6. **Session id.** Derive a short stable id (e.g. `roundtable-<slug>`); use it for the artifact directory and all filenames.
...
- the **domain glossary + relevant ADRs** from preflight (canonical vocabulary the seats must use and check against),
- any `context_files` the user pointed at (paths only — seats read them and the surrounding code read-only),
...
Never add your own interpretation, repo summaries, or a preferred reading to the brief. The glossary/ADRs are the project's own words, not your reading — those belong in.
...
`restated_request`, `intent`, `scope_in`, `scope_out`, `assumptions`, `ambiguities`, `terminology`, `code_findings`, `open_questions`, `confidence`.
...
You cannot see other models' answers. Argue from the request, the project's
domain glossary, and the actual code.

REQUEST TO INTERPRET:
<original prompt>

DOMAIN GLOSSARY & ADRs (canonical vocabulary — use these terms; flag any conflict):
<CONTEXT.md excerpts + relevant ADRs, or "none found">

CONTEXT FILES / CODE (read-only): <paths>
Cross-check your reading against the code; report contradictions, do not guess.

Return ONLY JSON matching this shape:
{restated_request, intent[], scope_in[], scope_out[], assumptions[], ambiguities[],
 terminology[{term, meaning, conflicts_glossary}], code_findings[], open_questions[], confidence}
```

`terminology` = key domain terms the seat relied on, each with its meaning and whether it conflicts with the glossary. `code_findings` = contradictions or confirmations the seat found by reading the code.
...
- **Disagreement points** — anything where seats differ on intent, scope, an assumption, an ambiguity's resolution, or **the meaning of a domain term**. Assign each a stable id (`D1`, `D2`, …) and a neutral one-line statement, plus each seat's current position on it.
- **Terminology** — terms the seats converged on a sharper canonical meaning for, and terms that conflict with the glossary (a term conflict is a `D#` point). The agreed sharpenings feed *Proposed glossary updates* in the report.
- **Code findings** — contradictions any seat surfaced between the prompt's assumptions and the actual code; carry these into the report's open questions if unresolved.
...
4. **Agreed interpretation** — the shared understanding (the locked agreement points), written as a clear restatement of what the user is asking for, in the project's canonical vocabulary.
...
6. **Clarifying questions for the user** — derived from the unresolved disagreements + code findings + shared open questions. Order them as a **decision tree** (resolve prerequisites first; note when one answer settles others), and for **each** give the competing readings **plus a moderator-recommended default and its rationale**, so the user can accept the defaults in one pass. (Recommended-answer style adapted from `grill-me`.)
7. **Proposed glossary updates** — terms the roundtable sharpened to a canonical meaning, or prompt terms that conflict with `CONTEXT.md`, in glossary form. This is a **read-only suggestion** — offer to apply it to `CONTEXT.md` (or hand it to the build); never write it yourself. Omit if there were none.
8. **Confidence** — `high` (≥3 seats, full or near-full consensus, language grounded in the glossary), `medium` (some durable disagreement, a missing seat, or no glossary to ground against), `low` (quorum barely met or major disagreement persists).

This skill ends at the shared interpretation; it does not implement anything. After delivering the report you may **offer** the handoff below — but never start building on your own.

## Handoff to implement-and-review

The report is exactly the input a build needs, so `model-roundtable | implement-and-review` is a natural pipeline (understand → build). When the run reaches `full_consensus` — or the user has answered the clarifying questions so no fork remains — **offer** (do not auto-run) to hand off to `implement-and-review`:

- Pass the **report path** (`.ai-workflow/roundtable/<session_id>/report.md`); the build consumes its **Agreed interpretation** as the settled task understanding.
- Do not hand off while **Unresolved disagreements / clarifying questions** remain — resolve them with the user first, or pass them through explicitly so the build's intake gate stops on them.
- If the user asked only for analysis, stop after the report; offer the handoff but do not initiate it.

**Handoff contract** (shared with `implement-and-review`): *Agreed interpretation* → settled task understanding that drives the FE/BE split; *Unresolved disagreements* + *Clarifying questions* → must be resolved before any code is written.
