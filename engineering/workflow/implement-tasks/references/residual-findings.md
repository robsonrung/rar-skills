...
Loaded by `implement-tasks` Phase 3 whenever review findings (from `full-review`, per task or feature-wide) were **not applied** to the code. Those residuals must become durable before delivery. `implement-tasks` is autonomous past the task approval, so this contract never prompts: the fallback chain executes silently and "no sink available" is a data-producing outcome, not a question.
...
_Adapted from Every's compound-engineering-plugin (`lfg`, `references/tracker-defer.md`)._
