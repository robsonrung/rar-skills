...
[ ] Grok (grok-runner)
[ ] Kimi (`pi-runner --seat kimi`)
[ ] GLM (`pi-runner --seat glm`)
[ ] Qwen (`pi-runner --seat qwen`)
[ ] Gemma (`pi-runner --seat gemma`)
[ ] Muse (`cline-runner --seat muse`)
```
...
**Bounded rounds:** `max_iterations` lives in `ceilings`, and the round number lives in `attempts.round`. Increment it in state _before_ launching the round, not after — a council that crashes mid-round has still spent that round. At the bound, stop and report the open divergence: **the model never decides the retry**, and it does not decide to award itself another round either.
