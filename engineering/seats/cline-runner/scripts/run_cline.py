...
# provides no model id to infer a vendor from (see infer_provider_from_model).
PROVIDER_BY_RUNNER = {
    "cline": "cline",
    "gemma": "google",
    "glm": "zai",
...
def normalize_envelope(
    ...
    # chain. Named seats (--seat muse|minimax) rely on this split to report
    # e.g. runner=muse, effective_runner=cline.
...
Examples:
  ...
  %(prog)s "Resume and continue" --session 1782865158637_s2n62
    ...
    parser.add_argument(
        "--seat",
        type=str,
        choices=sorted(CLINE_SEATS),
        default=None,
        help="Run as a named seat: pins that seat's model (unless --model overrides it) "
        "and labels the envelope runner=<seat>, effective_runner=cline. "
        + ", ".join(f"{seat}={model}" for seat, model in CLINE_SEATS.items()),
    )
    parser.add_argument(
        "--timeout",
        "-t",
...
def main(
    ...
    if args.seat:
        ...
        default_model = CLINE_SEATS[args.seat]
        default_model_by_provider = None
