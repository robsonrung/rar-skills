...
- `Settled decisions:` — session-settled Key Technical Decisions, or `none`. When Section 3 stress-testing or Section 5 alternative-blindness targets a listed decision, challenge it only on infeasibility grounds (evidence it cannot work), never on preference (you would have chosen otherwise) — settled decisions were made with the user in the room.

Run the full 5-technique protocol only when adversarial scrutiny is genuinely useful for that doc shape — when premise has already been settled upstream, several of the techniques re-litigate decided questions and produce noisy "the motivation is thin" findings on plans whose motivation lives in the linked requirements doc. Calibrate by combining the two slots:

**`Document type: requirements`:** primary home. Run the full 5-technique protocol per Depth calibration below. Premise and assumptions ARE the requirements doc's domain.
