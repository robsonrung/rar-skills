...
Use the anchored rubric from `../doc-findings-schema.json` (the `confidence` field's behavioral criteria). Feasibility's domain grounds in codebase evidence, so it reaches the strongest anchors when you can cite concrete technical constraints. Apply as:
