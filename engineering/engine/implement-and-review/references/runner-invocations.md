...
# (used by implement-tasks to isolate per-task builds):
...
For a standalone single task, omit `--slice`. The `--slice` namespacing exists so **`implement-tasks`** can run many per-task builds in parallel without collision; cross-task scheduling and integration live in that skill, not here.
...
Use the default Codex seat for product interfaces with an explicit component and behavior contract. Omit `--model`; the roster-backed runner default selects the current Codex seat.
...
The brief carries the literal frontend scope, acceptance commands, shared contracts, the Slice Contract snippet when the task is a `to-tasks` slice (expected review focus, rollback boundary), and the instruction to continue until the acceptance contract passes or a configured exit fires. Keep the returned session id for fix rounds.
...
EXPECTED REVIEW FOCUS (slice only):
<the slice's named risk — test it first>

ROLLBACK BOUNDARY (slice only):
<the slice's rollback note — introduce no side effect it does not declare>
...
Opus reviews work written by the Codex seat. Use a fresh native subagent when available, or `claude-runner --model opus --role codereviewer --effort medium --restrict-tools --disable-fallback`. Give it the complete task, the boundary around unchanged behavior, the diff, and the review output contract.
...
```bash
python3 .agents/skills/pi-runner/scripts/run_pi.py --seat kimi \
...
Every review prompt names the task, shared contracts, test evidence, and unchanged-behavior boundary — plus, for a slice, its expected review focus and rollback note. An implementer never reviews its own work.
...
| Fallback reviewer | `pi-runner --seat kimi --role codereviewer` | `pi-runner --seat kimi --role codereviewer` |
