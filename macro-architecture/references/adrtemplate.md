...
> **This file is the runtime template source, not the canonical one.**
> `architecture-lens/references/adr-template.md` is the repo-canonical ADR template — read it
> for the "is this decision architecturally significant?" test, the storage and superseding
> convention, and the writing tips. This file exists because `scripts/newadr.py` loads the
> fenced markdown block below at runtime to draft a file, so the template text must live in a
> machine-readable form here. Keep the two consistent: when the canonical template changes,
> mirror the change here.
>
> Two constraints on edits to the fenced block: the script rewrites the `# ADR: `, `Status: `,
> and `Date: ` lines by regex, so those three lines must keep exactly that shape and stay at
> the top. Everything else in the block is free text.
