...
| `--model`, `-m` | Compatibility metadata label. `agy` uses its configured model from `/model` or settings — this label is reflected in `effective_model` but not forwarded. The Gemini seat always runs `gemini-3.7-flash` (Gemini 3.7 Flash (High)). | `gemini-3.7-flash` |
...
- Model selection configured in `agy` itself (the wrapper's `--model` is a non-forwarded label). List the models `agy` accepts with `agy models`; the Gemini seat always runs **`Gemini 3.7 Flash (High)`**. Select it via `/model` in an interactive `agy` session or in `~/.gemini/antigravity-cli/settings.json` — the wrapper's `--model gemini-3.7-flash` only sets the `effective_model` label.
