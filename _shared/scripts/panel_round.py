#!/usr/bin/env python3
"""Run one role-panel phase for a panel-capable skill.
...
routing file is not `<skill>/assets/routing.toml` pass it with `--routing`; the
calling skill's root is then the routing file's parent directory's parent.
"""
...
def runner_script_tail(raw_path: str, runner: str | None) -> str | None:
    """Return the `<runner-skill>/scripts/run_<x>.py` tail for a runner wrapper.

    Derived from the configured path when it already carries that shape, and
    otherwise rebuilt from the provider's `runner` key.
    """
...
def resolve_runner_script(
    raw_path: str,
    runner: str | None,
    working_dir: Path,
    skill_root: Path,
) -> Path:
    """Locate a runner wrapper script, honoring the RUNNER_BASE_PATH override.

    Fallback chain, first existing path wins:

    1. the explicit `script` path from the routing file
    ...
    if raw_path:
        candidates.append(resolve_path(raw_path, working_dir, skill_root))
    tail = runner_script_tail(raw_path, runner)
    if tail:
        base = os.environ.get("RUNNER_BASE_PATH")
        if base:
...
def run_runner_role(
    *,
    ...
    if not script_raw and not runner_name:
        return {
            "role": role,
            "kind": "runner",
    ...
    script = resolve_runner_script(
        str(script_raw or ""),
        str(runner_name) if runner_name else None,
        working_dir,
        skill_root,
    )
    metadata = {
...
def main() -> int:
    ...
    parser.add_argument(
        "--routing",
        help="Path to the panel routing TOML (default: <skill_root>/assets/routing.toml).",
    )
    ...
    if args.routing:
        routing_path = Path(args.routing).expanduser().resolve()
        skill_root = routing_path.parents[1]
    else:
