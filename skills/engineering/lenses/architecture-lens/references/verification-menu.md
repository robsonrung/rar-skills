...
Concrete checks per quality attribute. Read this when a lens finding needs to become an executable check instead of an opinion. Choose checks matching the two or three quality attributes the change most affects — do not run every row.
...
A check that only a human can judge is not a verification step — it is a review note. When the concern will recur, promote the check to a fitness function (`fitness-functions.md`) so future changes cannot quietly break it.
