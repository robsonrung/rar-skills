...
When making a decision, you rarely optimize all of these — you pick the 2–4 that matter _here_ and accept worse scores on the rest. The job is to name the tension, not to maximize everything (that's impossible; characteristics conflict).

Use this list to ask: _"Which of these does this decision actually trade on?"_
...
3. That prioritization _is_ the justification for the option you pick. Write it down if the decision is significant (→ ADR).
...
- Ignoring an _implicit_ one the domain demands (e.g. auditability in finance, even if no ticket says so).
