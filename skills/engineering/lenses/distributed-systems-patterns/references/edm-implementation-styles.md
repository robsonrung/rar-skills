...
- **Microservice topology** — the processing graph _inside_ one service (filter, join, emit).
...
| External (DB, search, geo) | query shapes the broker cannot serve; FaaS/BPC | network latency; the _owning team_ runs it |
...
| **BPC** (plain producer/consumer) | sidecar into a legacy app; gating where arrival _order_ does not matter; the data layer does the work | multi-stream event-time order, local state scale-out |
...
**Choreography** — services react to public events. Default for _independent_ business workflows and for inter-team composition. Fragile when you must reorder the middle of a chain or see "where is order 123."

**Orchestration** — one service owns _only_ workflow sequencing and talks to workers that own their business rules. Default when the sequence changes, needs visibility, or is a saga. A god-orchestrator that retries payments is a boundary leak.
