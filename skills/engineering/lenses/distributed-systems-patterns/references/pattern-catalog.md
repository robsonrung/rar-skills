...
This file only decides _which Burns pattern names the topology_.

## Decision order

Walk top to bottom. Stop at the first yes. This _is_ the **smallest coherent shape** for this skill.
...
3. Otherwise it is serving (`references/serving.md`)
   ...
   2. Does a request need _all_ leaves, merged at a root, to finish faster or to cover sharded data? → scatter/gather
   ...
   5. Must _exactly one_ process own a task at a time? → **need a master**; singleton or election
...
Compose at most two in the brief; if you need more, the graph belongs in `roles` and the _headline_ pattern is the outermost one.
...
| "Should Kafka / events be the source of truth?" | the `event-driven` route of this skill |
| Retry, migration, **idempotent** write, transaction boundary in code | `data-systems-coding-lens` |
| Class/module structure, Strategy vs Decorator _in process_ | `design-patterns` |
