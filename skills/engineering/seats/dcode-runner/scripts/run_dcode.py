#!/usr/bin/env python3
"""
Dcode Runner - A wrapper for DeepAgents CLI (`dcode`) non-interactive execution.

Executes prompts using `dcode -n -q --no-stream`. The CLI's pre-configured model
and credentials are used as-is; the wrapper never selects a model or writes to
``~/.deepagents/``. Permission bypass (``dcode -y``) is opt-in via
``--auto-approve``. The file name and public function names stay stable for
existing dcode-seat workflows.
"""
...
# Substrings that indicate dcode could not authenticate. Matched against stderr
...
def resolve_restrict_tools(role: str | None, restrict_tools: bool, allow_write: bool) -> bool:
    ...
    ``--allow-write`` opts out, otherwise analysis roles default to read-only
    and a bare prompt with no role does not.
    """
    if restrict_tools:
        return True
    if allow_write:
...
def compute_timeouts(timeout: int) -> tuple[int, int]:
    """Return (dcode_timeout, subprocess_timeout) seconds.

    The dcode ``--timeout`` is set a little *below* the subprocess timeout so
    dcode terminates its own run cleanly (flushing partial output and returning
    its own 124 exit code) before the wrapper's hard ``subprocess`` timeout
    would SIGKILL it. The subprocess timeout stays equal to the caller's budget
    so the overall cap is honored.
    """
    total = max(1, int(timeout))
    buffer = max(1, min(10, total // 10))
    dcode_inner = max(1, total - buffer)
    return dcode_inner, total
...
def output_format_instruction(output_format: str) -> str:
    ...
    if output_format == "stream-json":
        return (
            "Response format request: dcode non-interactive mode is captured as "
...
def _annotate_fallback(
    ...
    fallback_result["fallback_from"] = runner_name
    fallback_result["fallback_reason"] = "DeepAgents CLI (dcode) not found"
    fallback_result["requested_model"] = requested_model or model
    fallback_result["fallback_model_forwarded"] = False
    fallback_result["dcode_continue_requested"] = dcode_continue
    fallback_result["resume_session_requested"] = resume_session
    ignored = []
    if dcode_continue:
        ignored.append("--dcode-continue")
    if resume_session:
        ignored.append("--resume")
    if ignored:
        fallback_result["fallback_ignored_options"] = ignored
    if attempted_fallbacks:
        fallback_result["fallback_attempts"] = attempted_fallbacks
    return fallback_result


def _fallback_attempt_record(
    script: Path, fallback_result: dict[str, Any], seat: str | None = None
) -> dict[str, Any]:
    return {
        "script": str(script),
        "seat": seat,
...
def _run_dcode_impl(
    ...
    def error_envelope(message: str, code: int, command: str = f"{DCODE_CLI} -n") -> dict[str, Any]:
        return {
            "success": False,
            "stdout": "",
            "stderr": message,
    ...
    if (dcode_continue or resume_session) and not final_prompt.strip():
        final_prompt = (
    ...
    if shutil.which(DCODE_CLI) is None:
        unavailable_env = {
            "success": False,
            "stdout": "",
            "stderr": "DeepAgents CLI (dcode) not found. Check if it is installed and in PATH.",
            "return_code": -2,
        ...
        for fallback_script, fallback_seat in fallback_candidates:
            if not fallback_script.is_file():
                attempted_fallbacks.append(
                    {"script": str(fallback_script), "seat": fallback_seat, "status": "not_installed", "return_code": None}
                )
                continue
            ...
            attempted_fallbacks.append(_fallback_attempt_record(fallback_script, fallback_result, seat=fallback_seat))
            if not (
    ...
    result: dict[str, Any] = {
        ...
        "dcode_inner_timeout": dcode_inner_timeout,
        "dcode_continue": dcode_continue,
        "resume_session": resume_session,
        "auto_approve": auto_approve,
        "max_turns": max_turns,
        "runner": runner_name,
        "effective_runner": "dcode",
        "role": role,
    ...
    try:
        ...
        # dcode exits 124 on either --timeout or --max-turns exceeded. Annotate
        # which one so a caller can tell a turn-cap exit from a wall-clock one.
        if process.returncode == 124:
            if max_turns is not None:
                result["max_turns_exceeded"] = True
            else:
        ...
        result["stderr"] = "DeepAgents CLI (dcode) not found. Check if it is installed and in PATH."
...
def run_dcode(
    ...
    """Execute a prompt using DeepAgents CLI (`dcode`) non-interactive mode.
    ...
    Args:
        ...
        timeout: Maximum wait time in seconds (default: 3600). dcode's own
            ``--timeout`` is set slightly below this so it self-terminates
            before the hard subprocess timeout.
        ...
        dcode_continue: Resume the most recent dcode session (``-r`` with no ID).
        resume_session: Resume a specific dcode session by id (``-r <id>``).
        ...
        auto_approve: Forward ``-y`` to dcode to skip human-in-the-loop prompts.
        max_turns: Cap dcode's agentic turns (forwarded as ``--max-turns``).
    ...
    return normalize_envelope(
        raw, requested_runner=runner_name, requested_model=(model or DEFAULT_MODEL)
    )
...
def main(
    ...
    parser = argparse.ArgumentParser(
        description=description or "Execute prompts using DeepAgents CLI (dcode) in non-interactive mode.",
...
Examples:
  ...
  %(prog)s "Apply the fix" --role implementer --auto-approve
        """,
    )
    ...
    parser.add_argument(
        "--prompt-file", action="append", default=None, dest="prompt_files",
        help="Read the prompt body from a file (repeatable; files are concatenated in order)",
    )
    parser.add_argument(
        "--restrict-tools", action="store_true",
    ...
    parser.add_argument(
        ...
        help="Forward dcode -y to skip human-in-the-loop permission prompts",
    )
    parser.add_argument(
        "--max-turns", type=int, default=None,
        help="Cap dcode's agentic turns (forwarded as --max-turns)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Requested model label for metadata. dcode uses its already-configured "
            f"model; this label is NOT forwarded (default metadata: {default_model})"
        ),
    )
    parser.add_argument(
        "--output-format", "-o", type=str,
        choices=["text", "json", "stream-json"], default="text",
        help="Requested response format hint (dcode -n emits plain text only)",
    )
    parser.add_argument(
        "--role", type=str,
        choices=sorted(ROLE_INSTRUCTIONS), default=None,
    ...
    parser.add_argument(
        "--dcode-continue", action="store_true",
        help="Pass -r to dcode to resume the most recent dcode session",
    )
    parser.add_argument(
        "--resume", type=str, default=None, metavar="SESSION_ID",
        help="Pass -r <SESSION_ID> to dcode to resume a specific session by id",
    )
    parser.add_argument(
        "--disable-fallback", action="store_true",
        help="Do not route to another runner if DeepAgents CLI (dcode) is unavailable",
    )
    parser.add_argument(
    ...
    if not args.prompt and not args.prompt_files and not (args.dcode_continue or args.resume):
        parser.error("Provide a prompt argument, --prompt-file, --dcode-continue, or --resume")

    if args.prompt and args.prompt_files:
        parser.error("Provide either a positional prompt OR --prompt-file, not both")

    if args.dcode_continue and args.resume:
        parser.error("Provide either --dcode-continue OR --resume, not both")
    ...
    result = run_dcode(
        ...
        dcode_continue=args.dcode_continue,
