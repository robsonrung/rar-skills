...
class FallbackSelectionTests(unittest.TestCase):
    def test_skips_unavailable_to_first_success(self):
        ...
        def fake_invoke(script, *args, **kwargs):
            name = kwargs.get("seat") or Path(script).name
            calls.append(name)
            if name == "qwen":
    ...
    def test_full_fallback_order_is_qwen_kimi_codex_claude(self):
        ...
        def fake_invoke(script, *args, **kwargs):
            calls.append(kwargs.get("seat") or Path(script).name)
            return {"success": False, "return_code": -2, "status": "seat_unavailable"}
        ...
        self.assertEqual(
            calls,
            ["qwen", "kimi", "run_codex.py", "run_claude.py"],
    ...
    def test_first_success_short_circuits_remaining_fallbacks(self):
        ...
        self.assertEqual(calls, ["qwen"], "later fallbacks must not run after a success")
        self.assertTrue(env["success"])
