...
- **GPT 5.6 Sol / Codex missing (BE implementer):** first try **Kimi K3** as the fallback BE implementer (`pi-runner --seat kimi --role implementer --effort high`, write-enabled; Opus still reviews — cross-vendor diversity is preserved) and note the substitution — Kimi is the stronger coding seat of the two write-capable fallbacks (Terminal-Bench, ProgramBench, FrontierSWE). If `pi`/Kimi is also missing, try **Grok 4.6** (`grok-runner --role implementer --effort high`, write-enabled) next and note the substitution. If both are missing, with approval have an Opus subagent implement BE and a _different_ model review it; flag lost cross-vendor diversity. If no write-capable seat + distinct reviewer remain for a non-empty track, stop and report.
