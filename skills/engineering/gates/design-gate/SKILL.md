...
5. On `revise`, restate the required plan changes as a short numbered list. On disagreement between lenses about a real trade-off, escalate per the ladder in `implement-tasks` (multi-model consensus, then logged assumption) instead of asking the user.
