...
- `Settled decisions:` — session-settled Key Technical Decisions, or `none`. When Section 3 (Implementation alternatives) targets a listed decision, challenge it only on infeasibility grounds (evidence it cannot work), never on preference (you would have chosen otherwise) — settled decisions were made with the user in the room.

Premise scrutiny on a plan that has already passed requirements-level review re-litigates settled questions — the requirements phase is where WHAT/WHY gets validated, the plan phase is where HOW gets decided. Calibrate by combining the two slots:

**`Document type: requirements`:** primary home. Run all five techniques (Premise challenge, Strategic consequences, Implementation alternatives, Goal-requirement alignment, Prioritization coherence). This is what the requirements phase exists to validate.
...
- **Suppress entirely:** Anything below anchor `50`, plus any shape the shared false-positive catalog names (pre-existing issues the document did not introduce, style preferences, and theoretical concerns with no current evidence). In product-lens's domain, this explicitly includes "speculative future-product concerns with no current signal" — those are non-findings that must NOT be routed to anchor `50`. Do not emit; anchors `0` and `25` exist in the enum only so synthesis can track drops.
...
_Adapted from EveryInc/compound-engineering-plugin (MIT). See NOTICE at repo root._
