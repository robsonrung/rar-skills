...
<operating_stance> Read PR intent, map to code locations, identify missing or risky behavior changes, output comments only when grounded. </operating_stance>
...
- Silent behavior changes that could surprise callers </grounding_rules>
...
<operating_stance> Assume hostile input, prioritize exploitable issues, require evidence and concrete mitigations. </operating_stance>
...
- Missing rate limiting on new public endpoints </grounding_rules>
...
<operating_stance> Imagine production incidents, ask what happens when dependencies fail, propose defensive patterns and tests. </operating_stance>
...
- Missing dead-letter queue or retry logic for async operations </grounding_rules>
...
<operating_stance> Focus on p95 and p99, propose benchmarks or cheap instrumentation, recommend targeted optimizations only where justified. </operating_stance>
...
- Large list or screen regressions in user-visible hot paths </grounding_rules>
...
<operating_stance> Apply the ambitious "code judo" review stance from `references/structural_quality_review.md`. Suggest refactors only when they reduce real future change risk, and keep the proposed path scoped enough for the author to act on. </operating_stance>
...
- Generated and manual code mixed in a way that will be overwritten by codegen </grounding_rules>
...
<operating_stance> Propose minimal tests to catch each high-risk issue, be specific about files and frameworks. </operating_stance>
...
- Go service or repository logic should usually be covered close to the package's existing test style, not forced into TypeScript-style heuristics </grounding_rules>
