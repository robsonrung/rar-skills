...
Create one immutable handoff at the destination the user requested, or use the managed store by default. Briefly summarize what the handoff captured, then report its final path, retention limits, and continuity warnings. The handoff supplements authoritative artifacts; it does not replace them. Never write a shared "latest" file: every handoff gets its own path, and existing handoffs are never overwritten.
...
When the user did not choose another destination, the managed store is `.ai-workflow/handoff/` under the repository's **main** worktree, so every worktree of the same repository reads and writes one store (every skill in this collection keeps its working files under `.ai-workflow/`). Resolve it with this shell block:
...
HANDOFF_DIR="$REPO_ROOT/.ai-workflow/handoff";
if [ -L "$HANDOFF_DIR" ]; then echo "unsafe handoff dir symlink: $HANDOFF_DIR" >&2; exit 1; fi;
(umask 077; mkdir -p "$HANDOFF_DIR") || exit 1; chmod 700 "$HANDOFF_DIR" || exit 1;
```

Write a Markdown snapshot at `$HANDOFF_DIR/<topic>.md`.

Use a readable topic slug as the filename. Outside a Git repository, `$PWD/.ai-workflow/handoff/` is the store and `general` is the namespace recorded in frontmatter. Worktrees from the same repository share the store and remain distinguishable through frontmatter (`branch`, `worktree_path`). If `.ai-workflow/` is not yet ignored by the repository, say so in the report rather than editing `.gitignore` unasked. Do not put a timestamp or unique ID in the path by default; `created_at` carries chronology for discovery.

Reserve the final candidate filename atomically and exclusively (for example `set -C; : > "$path"` in a fresh shell, or another O_EXCL-equivalent create); on collision, retry with the smallest available numeric suffix rather than overwrite a handoff. Never check availability and then write — reserve, or pick a new name. Keep the directory and file user-private where the platform supports permissions.

Treat creation as complete only after confirming the destination contains the handoff. Give a succinct, context-specific summary of what the generated handoff captures so the user can verify its substance without opening it; do not impose a fixed summary template. Then report the final path and any warnings together. The managed store lives inside the repository checkout, so a receiving session on the same checkout (or any worktree of it) discovers the handoff automatically; a session elsewhere needs the file transferred to a receiver-visible location and resumed from that explicit source.

End the creation response with one fenced, copyable resume command using the final path:

```text
/session-handoff resume <final path>
```

Quote the path when needed so the command can be pasted verbatim. Do not generate a longer resume prompt.
...
```yaml
---
artifact_contract: "session-handoff/v1"
...
Treat a supplied local file, pasted document, or other specific artifact as the user's selection. Read that source, then follow **Orient from the selected source**. Do not require it to have been written by this skill or to use `session-handoff/v1`; authorship, ownership, location, and format are not eligibility gates. Do not search for an alternative automatically. If the source cannot be read, explain the access problem and ask the user for a reachable source or different direction.
...
1. Search the folder or collection the user supplied; otherwise run the managed-root block above in the current shell call and enumerate candidate files beneath `$HANDOFF_DIR/`. Bound the candidate set before inspecting content; prefer recent files and current repository or working-directory affinity without making repository affinity mandatory.
...
_Provenance: ported from the upstream `ce-handoff` skill; the store layout, frontmatter contract, and bounded-discovery resume flow follow that design, renamed under the `session-handoff/v1` contract._
