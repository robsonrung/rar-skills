...
`schema.yaml` in this directory is the canonical contract for `docs/solutions/` frontmatter written by `capture-learning`.
...
- **component**: One of `cli`, `api`, `service`, `background_job`, `database`, `frontend`, `backend`, `build_system`, `ci_pipeline`, `testing_framework`, `development_workflow`, `documentation`, `tooling`, `authentication`, `integration`, `skill`, `script`
...
- Bug-track fields present on a knowledge-track doc are harmless. Do not strip them unless the doc is being rewritten for other reasons.
...
This rule applies to all array-of-strings frontmatter fields. Scalar string fields (like `title:`) are covered by the parser-safety checks in `scripts/validate-frontmatter.py`: quote any scalar value containing ` #` or `: `.
