...
Put the actual logic (the bit that's answering the question) in a single `<script>` block written as a small, pure module whose decisions can be explained independently of the page. Both the module and the page are throwaway.
...
Pick whichever shape best fits the question being asked, _not_ whichever is easiest to wire to a page. Keep it pure: no DOM, no `document`, no button handlers reaching inside it. The page calls into it; nothing flows the other direction. Once the question is answered, record the selected behavior and rebuild it in the production task with its required tests and error handling.
...
Once the prototype has answered its question, capture the answer and observations as the [SKILL.md](../SKILL.md) describes. Keep the local HTML artifact as evidence when useful. Production work rebuilds the selected behavior; neither the logic nor the page is promoted.

## Anti-patterns

- **Keep checks specific to the question.** Add assertions when needed to trust the observation; omit a production test suite.
...
- **Do not ship prototype code.** Record the selected behavior and rebuild it under the production acceptance contract.
