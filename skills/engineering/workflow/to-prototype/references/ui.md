...
Once the user chooses a direction, record which variant and why. Keep the local prototype as evidence as [SKILL.md](../SKILL.md) describes. Return the decision to the caller; a later production task rebuilds it with required validation. Neither the selected variant nor the switcher is promoted.

Remove prototype routes or switches only from the work created for this experiment. Keep unrelated edits. A throwaway commit or tracker update requires authorization.
