...
The model roster is the source of truth for seat ids, current model ids, providers, and transports: `shared/references/model-roster.md`. The task-shaped routing reference selects recommended seats and effort. `shared/references/host-model-execution.md` selects native delegation before an external runner. This skill never pins a model id in its prose or command examples.
