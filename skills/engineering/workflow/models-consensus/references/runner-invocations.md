...
The consensus Gemini seat does architecture/synthesis reasoning, so it runs **Gemini 3.8 Flash (High)** (`--model` is a metadata label — set agy's own model picker to match). Do not depend on speculative Gemini-only flags such as `--thinking-budget` or a read-only convenience mode.
...
`--seat kimi` pins the Moonshot model per invocation through `pi` on OpenRouter — omit `--model` rather than pinning the id here. The only credential is `OPENROUTER_API_KEY`; there is no lane provisioning, no shared provider state, and Kimi/GLM rounds launch in parallel safely.

### GLM

```bash
python3 .agents/skills/pi-runner/scripts/run_pi.py --seat glm \
