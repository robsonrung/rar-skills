...
Read and follow this file only when browser-smoke is invoked with `mode:pipeline` — in this repo that caller is an orchestrator's verify step for web-facing changes (`implement-and-review` does not call it). It overrides interactive prompts, free-port selection, and dev-server startup. It does not change browser-driver selection. In pipeline mode you run unattended — never block on a question.

## 1. No interactive prompts

Unattended execution does not mean hidden execution. Do not ask any visibility or how-to-proceed question:

- When the in-app Browser pane is the selected driver, its surface stays visible and non-blocking so the user can watch progress without interrupting the run. Do not repeatedly steal focus.
- When the playwright MCP fallback is selected, it runs its own managed browser; no prompt is needed.
- Human-verification flows (OAuth, email, payments, SMS) are logged as Skip with the reason; failures are captured and logged, then the run continues.
...
mkdir -p .ai-workflow/browser-smoke
LOG=".ai-workflow/browser-smoke/dev-server-${PORT}.log"
echo "Starting dev server on port ${PORT}..."
if [ -f "bin/dev" ]; then
  PORT=${PORT} bin/dev > "$LOG" 2>&1 &
