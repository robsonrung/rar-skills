...
The cmux terminal sets `CMUX_WORKSPACE_ID` and `CMUX_SURFACE_ID`. The CLI uses `CMUX_SOCKET_PATH` when it is set.
...
Supported keys are `enter`, `tab`, `escape`, `backspace`, `delete`, `up`, `down`, `left`, and `right`.
...
The default release socket is `/tmp/cmux.sock`. Socket access is normally limited to processes launched inside cmux. A socket request is newline-delimited JSON such as:

```json
{ "id": "workspace-list", "method": "workspace.list", "params": {} }
```

Legacy payloads with `command` are not supported. `CMUX_SOCKET_MODE=allowAll` permits any local process to connect, so retain the default scope unless the user explicitly authorizes the wider access.
