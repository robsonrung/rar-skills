...
For each prescriptive block ask: _if this instruction disappears, can the workflow produce a wrong path, state, count, gate, field, boundary, coverage floor, or handoff?_ If yes it is **protocol** — keep it explicit and falsifiable. If removal mainly gives a capable model more freedom to reason, it is **judgment** — first try deleting it; if observed behavior shows it is needed, compress it to the smallest principle or contrast pair.
