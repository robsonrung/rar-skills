...
Self-contained: this skill runs its own loop and never calls the council skill (`models-consensus`, any mode). It _does_ drive the **runner skills** (`claude-runner`, `codex-runner`, `gemini-runner`, `grok-runner`, and the Kimi and GLM seats of `pi-runner`) and the repo's **lens skills** — those are its building blocks.

## Best model for each task

Assign work to the model best suited to it. This is the default routing; fall back per the rules below when a seat is missing. Read `shared/references/task-shaped-model-routing.md` before changing these assignments.

| Task | Model (seat) | How to launch | Why this model |
| --- | --- | --- | --- |
| Branch: **smallest viable change** | Kimi K3 | `pi-runner --seat kimi --role planner` | pragmatic, code-native, strong long-horizon coding judgment |
...
- A seat marked `unavailable` → reassign its task to the next-best **available** model (e.g. no Kimi → smallest-change branch goes to Sonnet or GLM; no GLM → robust branch goes to Codex; no Grok → the 4th branch falls back to Gemini 3.8 Flash (`gemini-runner --role planner`); no Codex → data/security lenses + synthesis go to Opus).
...
- **Different boundary/placement** (4th, only when _where_ the code should live is itself in question).
...
Run one hard skeptical pass with the default Codex seat at high effort. Give it the chosen scope, acceptance contract, relevant commands, and this question: _what angle, edge case, model, or applicable lens did we still not cover, and can an implementer execute this plan without making a design decision?_
