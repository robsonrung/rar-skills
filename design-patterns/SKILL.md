...
description: >-
  Recognize when a Gang-of-Four design pattern genuinely fits the code at hand, name it in shared vocabulary, and translate it into a concrete design without overengineering. Use when choosing how to structure new code, when a design feels rigid or repetitive (sprawling if/switch, duplicated behavior, tangled notifications), or when someone asks "which pattern fits here". Triggers on "design pattern", "strategy/observer/decorator/factory/adapter/state/etc. pattern", "make this extensible", "this is getting hard to change". Distinct from software-design-philosophy (module-level complexity and conceptual integrity) and macro-architecture (it owns the macro style and architecture-pattern levels; this skill owns the source-code level). Do NOT use for pure bug-fixing, formatting, or when a plain function already does the job.
---
