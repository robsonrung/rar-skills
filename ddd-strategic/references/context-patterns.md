...
Detailed reference for the `ddd-strategic` skill. Source: *Learning Domain-Driven Design*,
...
- **Outbox** — atomically commit state + outgoing events, relay publishes reliably. Prevents
  lost or premature events. (See `ddd-tactical` for the algorithm.)
...
Strategic drift should pull tactical patterns with it (see `ddd-tactical` Lens 6).
