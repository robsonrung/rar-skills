...
When you describe your work, use the leitwort: “The decision surface leads with the outcome and keeps unresolved evidence visible.” Use **lead with the outcome** as the first layout rule, not as a late summary note.
...
When a browser is available, follow the verification steps in the shared conventions (desktop and narrow widths, expand controls, every navigation link reaches a real section) and additionally confirm that open divergence remains visible on the first screen. If no browser is available, report that the mechanical check passed but visual inspection was not available.
