...
5. **Unnecessary existence checks**: pre-checking file/resource existence before operating (TOCTOU anti-pattern) — operate directly and handle the error. Exception: checks that are themselves a security control at a trust boundary are not removable — flag the TOCTOU risk instead of the check.
