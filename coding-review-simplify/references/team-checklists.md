...
Read this at the end of a review when the change is about to be handed off or deployed. Use
only the items that are easy to miss and not already automated — anything a linter, type
checker, or CI job already enforces does not belong on a manual list. If an item here can be
automated, automate it and delete the row.
...
A release item with no named owner is not a plan. If the answer to "who does this if it goes
wrong" is nobody, that is the finding to report.
