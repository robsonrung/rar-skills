...
Availability is never assumed: `_shared/scripts/discover_runners.py probe` reports `available`, `cli_path`, `version`, `blocked_reason`, and `depends_on` per seat, and that envelope *is* the seat table. A seat whose CLI is missing, whose credentials are absent, or whose cheap headless smoke test fails is a **blocker**, not a soft warning — drop the seat, lower confidence, say so. Per-seat auth and transport rules (Claude's `--bare` caveat, the `agy` model picker, `grok login`, the shared `cline` provider for Kimi, GLM, Qwen, Gemma, and Minimax, and the mandatory `--disable-fallback`) live in [runner-invocations.md](runner-invocations.md).
