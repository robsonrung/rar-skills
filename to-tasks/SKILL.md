...
If an issue tracker is configured, publish each approved slice as an issue using the template below, with the `ready-for-agent` triage label, in dependency order (blockers first) so real identifiers can be referenced in "Blocked by". If no tracker is configured, write the approved breakdown to a `TASKS.md` at the repo root using the same template, one section per task, with a status line (`todo | in-progress | done | blocked`).
...
- Behaviors: each observable behavior that must hold
